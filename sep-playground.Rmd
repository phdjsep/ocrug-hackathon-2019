...
act_vio %>% 
  ...
  ggplot(aes(long, lat, group = group, fill = log(n))) +
...
act_vio %>% 
  group_by(county, year = year(viol_begin_date)) %>% 
  count() %>% 
  ungroup() %>% 
  right_join(ca_counties, by = c("county" = "county_name")) %>% 
  mutate(n = replace_na(n, 0),
...
act_vio %>% 
  count(violation_type_name) %>% 
  arrange(desc(n)) %>% 
  head(10) %>% 
  kable(format = 'markdown')

# Most common analytes in violation
act_vio %>% 
  count(analyte_name) %>% 
  arrange(desc(n)) %>% 
  head(10) %>% 
...
# What analytes show greatest departure from acceptable level
act_vio %>% 
  mutate(above_threshold = result_value - mcl_value) %>% 
  group_by(analyte_name) %>% 
  summarize(median_above_threshold = median(above_threshold, na.rm = TRUE)) %>% 
  arrange(desc(median_above_threshold)) %>% 
  head(10) %>% 
  kable(format = 'markdown')
```
